...
- 🗑️ **批量删除**：支持选择多张图片一次性删除，提高效率
- 📄 **分页显示**：优化大量图片的显示和处理，提升性能
- 💾 **多次保存**：保存选中图片后，未选中的图片会保留，可多次保存
...
6. 在审核页面中选择要保留的截图，点击"保存选中图片"按钮
7. 合并后的HTML文件将保存到您的默认下载位置
8. 未选中的图片会保留在审核面板中，可以继续选择保存或删除
9. 可以使用"删除选中图片"按钮批量删除不需要的截图
10. 如需手动审核，可随时点击控制面板上的"审核截图"按钮
